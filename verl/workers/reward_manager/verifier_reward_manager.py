...
class VerifierRewardManager:
    ...
    def process_single(self, args):
        ...
        if isinstance(result, dict):
            ...
            score, acc_score, format_score, bbox_tool_call_score = result
            time_end = time.time()
            ...
            return (index, score, acc_score, format_score, bbox_tool_call_score, valid_response_length, data_source, prompt_str, response_str, ground_truth, time_end - time_start)
    ...
    def __call__(self, data: DataProto):
        ...
        bbox_tool_call_reward_tensor = reward_tensor.clone()
        ...
        for i, result in enumerate(results):
            if isinstance(result[0], dict) and result[0]['is_filter'] == True:
                ...
                continue
            (score, acc_score, format_score, bbox_tool_call_score, valid_response_length, data_source, prompt_str, response_str, ground_truth, time_consume) = result
            ...
            bbox_tool_call_reward_tensor[i, valid_response_length - 1] = bbox_tool_call_score
            ...
            if already_print_data_sources[data_source] < 5:
                ...
                print("[score]", (score, acc_score, format_score, bbox_tool_call_score))
        ...
        return reward_tensor, acc_reward_tensor, format_reward_tensor, bbox_tool_call_reward_tensor, invalid_uids
