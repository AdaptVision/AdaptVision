...
class NaiveMultiThreadsBboxRewardManager:
    """The reward manager.
    """
    ...
    def process_single(self, args):
        ...
        extra_info_from_data = data_item.non_tensor_batch.get('extra_info', None)
        # Merge extra_info correctly without mutating original references
        extra_info = dict(self.extra_info)
        if isinstance(extra_info_from_data, dict):
            extra_info.update(extra_info_from_data)

        # Inject bbox image width/height from current sample if available
        try:
            ...
            if isinstance(multi_modal_data, dict):
                ...
                if isinstance(images, list) and len(images) > 0:
                    extra_info['image'] = images[0]  
                    w, h = images[0].size
                    extra_info['bbox_image_width'] = w
                    extra_info['bbox_image_height'] = h
        except Exception:
            pass
        ...
        if isinstance(result, dict):
            ...
            score, acc_score, format_score, area_penalty_score, area_ratio, bbox_score, tool_call_count = result

            time_end = time.time()

            return (index, score, acc_score, format_score, area_penalty_score, area_ratio, bbox_score, tool_call_count, valid_response_length, data_source, prompt_str, response_str, ground_truth, time_end - time_start)
    ...
    def __call__(self, data: DataProto):
        ...
        area_penalty_score_tensor = reward_tensor.clone()
        area_ratio_tensor = torch.zeros_like(data.batch['responses'], dtype=torch.float32)
        bbox_score_tensor = reward_tensor.clone()
        tool_call_count_tensor = torch.zeros_like(data.batch['responses'], dtype=torch.float32)
        ...
        for i, result in enumerate(results):
            ...
            (score, acc_score, format_score, area_penalty_score, area_ratio, bbox_score, tool_call_count, valid_response_length, data_source, prompt_str, response_str, ground_truth, time_consume) = result
            ...
            area_penalty_score_tensor[i, valid_response_length - 1] = area_penalty_score
            area_ratio_tensor[i, valid_response_length - 1] = area_ratio
            bbox_score_tensor[i, valid_response_length-1] = bbox_score
            tool_call_count_tensor[i, valid_response_length - 1] = tool_call_count
            ...
            if already_print_data_sources[data_source] < 5:
                ...
                print("[score]", (score, acc_score, format_score, area_penalty_score, bbox_score))
        ...
        return reward_tensor, acc_reward_tensor, format_reward_tensor, area_penalty_score_tensor, area_ratio_tensor, bbox_score_tensor, tool_call_count_tensor, invalid_uids
