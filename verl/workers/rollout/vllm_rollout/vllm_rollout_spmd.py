...
class vLLMRollout_MultiTurn_ToolCall(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        max_image_num = self.config.get('limit_mm_per_prompt', {'image': 1})['image'] # 2
        max_iterations = self.max_generation_round # 2
        ...
        with tqdm(total=worker_trajs_count, desc="Worker Rollout Progress", unit="task") as pbar:
            ...
            while current_iteration < max_iterations and len(to_generate) > 0: 
                ...
                for i_gen, response_ in zip(to_generate, response):
                    ...
                    tool_call_contents = pattern.findall(decoded_resp_)  # 看 decode 出来的东西是否包含 tool_call
                    if len(tool_call_contents) > 0:
                        ...
                        try:
                            ...
                            tool_type = prepare_tool_call_inputs(json_objects)  # = resize
                        except Exception as e:
                            print(str(e))
                            error_info = str(e)
                            tool_type = None
                        id_tool_query_mapping[str(i_gen)] = {
                            "tool_type": tool_type,  # = resize
