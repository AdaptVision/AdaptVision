...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        if self.config.control_alpha != 0:
            select_keys.append('tool_call_mask')
            select_keys.append('direct_answer_mask') 
            select_keys.append('turn_mask')       
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    control_alpha = self.config.control_alpha
                    tool_call_mask = None
                    direct_answer_mask = None
                    turn_mask=None
                    if control_level in ['turn_level', 'tool_level']:
                        tool_call_mask = data['tool_call_mask'][:, -response_length:]
                        direct_answer_mask = data['direct_answer_mask'][:, -response_length:]
                        turn_mask = data['turn_mask'][:, -response_length:]
