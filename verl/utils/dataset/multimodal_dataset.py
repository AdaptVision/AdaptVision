...
prompt_template_dict = {
    ...
    "image_crop_agent": "Answer the question based on the image provided. You must conduct reasoning within <think> and </think> first in each of your reasoning steps. You may call ONE function tool per step to help you better solve the problem. Place the function tool within <tool_call> and </tool_call> at the end of each step to perform a function call. You should continue your reasoning process within <think> and </think> based on the content returned by the function tool. Once you confirm your final answer, place the final answer inside <answer> and </answer>. For mathematical or multiple-choice problem, wrap the answer value or choice with \\boxed{{}}. Here is the image and question:\n{Question}",
    "image_crop_agent_with_hw": "Answer the question based on the image provided. You must conduct reasoning within <think> and </think> first in each of your reasoning steps. You may call ONE function tool per step to help you better solve the problem. Place the function tool within <tool_call> and </tool_call> at the end of each step to perform a function call. You should continue your reasoning process within <think> and </think> based on the content returned by the function tool. Once you confirm your final answer, place the final answer inside <answer> and </answer>. For mathematical or multiple-choice problem, wrap the answer value or choice with \\boxed{{}}. The width and height of the input image is {Width} and {Height}. The coordinates of the bounding box must satisfy the condition: 0 <= x1 < x2 <= {Width}, 0 <= y1 < y2 <= {Height}, x2 - x1 >= 28, y2 - y1 >= 28. Here is the image and question:\n{Question}",
}
...
def process_image(image: dict, max_pixels: int = 2048 * 2048, min_pixels: int = 512 * 512, tool_call=False, use_tgt_size=False, tgt_w=None, tgt_h=None, use_tool=False, factor=None):
    ...
    if tgt_w and tgt_h:     
        ...
        if factor is not None:
            height,width = smart_resize(height=image.height, width=image.width, factor=factor, min_pixels=4*factor*factor, max_pixels=2048*2048)
    ...
    if factor is not None:
        #  factor: Output dimensions will be multiples of this value.
        height, width = smart_resize(height=image.height, width=image.width, factor=factor, min_pixels=4*factor*factor, max_pixels=2048*2048)
        image = image.resize((width, height), resample=Image.Resampling.LANCZOS)
        print(f"data:Resized image to {width}x{height}")
...
class MultiModalDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        def add_prompt(example):
            template = prompt_template_dict[self.prompt_type]
            if '{Width}' in template or '{Height}' in template:
                problem = template.format(Question=example["problem"], Width="{Width}", Height="{Height}")
            else:
                problem = template.format(Question=example["problem"])
            example["prompt"] = [
    ...
    def __getitem__(self, item):
        ...
        if self.image_key in row_dict:  # expand image token
            ...
            try:
                row_dict['multi_modal_data'] = {'image': [process_image(row_dict[self.image_key], self.max_pixels, self.min_pixels, self.tool_call, self.use_tgt_size, tgt_w, tgt_h, use_tool, factor=28)]}
            ...
            # 如果prompt模板包含Height和Width占位符，重新生成prompt
            if '{Height}' in prompt_template_dict[self.prompt_type] or '{Width}' in prompt_template_dict[self.prompt_type]:
                problem = prompt_template_dict[self.prompt_type].format(
                ...
                prompt_with_chat_template = self.tokenizer.apply_chat_template(updated_chat, add_generation_prompt=True, tokenize=False)
                raw_prompt = prompt_with_chat_template.replace('<image>', '<|vision_start|><|image_pad|><|vision_end|>')
            else:
                # 对于不包含Width和Height的模板，检查是否需要替换占位符
                if '{Width}' in prompt_with_chat_template or '{Height}' in prompt_with_chat_template:
                    prompt_with_chat_template = prompt_with_chat_template.replace('{Width}', str(input_width)).replace('{Height}', str(input_height))
                    raw_prompt = prompt_with_chat_template.replace('<image>', '<|vision_start|><|image_pad|><|vision_end|>')
            ...
            if '{Width}' in prompt_with_chat_template or '{Height}' in prompt_with_chat_template:
                prompt_with_chat_template = prompt_with_chat_template.replace('{Width}', '0').replace('{Height}', '0')
        ...
        if self.use_raw_image:
            ...
            try:
                row_dict['multi_modal_data']['image'].extend([process_image(row_dict[self.image_key], self.max_pixels, self.min_pixels, self.tool_call, self.use_tgt_size, tgt_w * 2, tgt_h * 2, use_tool, factor=28)])
            except Exception as e:
                print(str(e))
                return self.__getitem__(item+1) if item + 1 < len(self) else self.__getitem__(0)
        ...
        if self.general_qa_reward_fn in ["general_qa_gpt", "general_qa_verifier", "general_qa_bbox"]:
        ...
        if input_height is not None and input_width is not None:
            row_dict['input_height'] = input_height
            row_dict['input_width'] = input_width
