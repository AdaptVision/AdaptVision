...
def compute_data_metrics(batch, use_critic=True):
    ...
    if 'format_scores' in batch.batch:
        ...
        bbox_tool_call_reward = batch.batch['bbox_tool_call_scores']
    else:
        format_reward = torch.tensor(0.0)
        accuracy_reward = torch.tensor(0.0)
        bbox_tool_call_reward = torch.tensor(0.0)
    ...
    metrics = {
        ...
        'critic/rewards/mean/bbox_tool_call':
            torch.mean(bbox_tool_call_reward).detach().item(),
        'critic/rewards/mean':
...
class RayPPOTrainer(object):
    ...
    def _maybe_log_tool_call_metrics(self, batch, metrics):
        ...
        apply_resize_tool_call_list = []
        apply_bbox_tool_call_list = []
        for response, multi_modal_item in zip(responses, multi_modal_data):
            if self.config.actor_rollout_ref.rollout.name in ["vllm_multi_turn_tool_call", "vllm_multi_turn_crop_call", "vllm_multi_turn_multi_tool"]:
                tool_call_pattern = re.compile(r'<tool_call>(.*?)</tool_call>', re.DOTALL)
                apply_tool_call = len(tool_call_pattern.findall(response)) > 0
                apply_resize_tool_call = "request_high_res_image" in response
                apply_bbox_tool_call = "request_local_region" in response
            ...
            apply_resize_tool_call_list.append(apply_resize_tool_call)
            apply_bbox_tool_call_list.append(apply_bbox_tool_call)
        ...
        if batch.meta_info.get('validate', False):  # Validation
            ...
            metrics['val/tool_call/apply_resize_tool_call_ratio'] = sum(apply_resize_tool_call_list) / len(apply_resize_tool_call_list)
            metrics['val/tool_call/apply_bbox_tool_call_ratio'] = sum(apply_bbox_tool_call_list) / len(apply_bbox_tool_call_list)
        else:   # Train
            ...
            metrics['tool_call/apply_resize_tool_call_ratio'] = sum(apply_resize_tool_call_list) / len(apply_resize_tool_call_list)
            metrics['tool_call/apply_bbox_tool_call_ratio'] = sum(apply_bbox_tool_call_list) / len(apply_bbox_tool_call_list)
    ...
    def _validate(self):
        ...
        if self.config.reward_model.log_rewards_separately:
            ...
            bbox_tool_call_reward_tensor_lst = []
        ...
        for test_data in self.val_dataloader:
            ...
            reward_tensor, acc_reward_tensor, format_reward_tensor, bbox_tool_call_reward_tensor, invalid_uids = self.val_reward_fn(test_batch)
            if self.config.reward_model.log_rewards_separately:
                format_reward_tensor_lst.append(format_reward_tensor)
                acc_reward_tensor_lst.append(acc_reward_tensor)
                bbox_tool_call_reward_tensor_lst.append(bbox_tool_call_reward_tensor)
            # Store scores
        ...
        if self.config.reward_model.log_rewards_separately:
            ...
            bbox_tool_call_reward_tensor = torch.cat(bbox_tool_call_reward_tensor_lst, dim=0).sum(-1).cpu()  # (batch_size,)
            if self.tool_call:
                penalized_acc_reward_tensor = reward_tensor - format_reward_tensor - bbox_tool_call_reward_tensor
        ...
        if self.config.reward_model.log_rewards_separately:
            ...
            data_source_bbox_tool_call_reward = {}
            if self.tool_call:
            ...
            for i in range(reward_tensor.shape[0]):
                ...
                if data_source not in data_source_bbox_tool_call_reward:
                    data_source_bbox_tool_call_reward[data_source] = []
                data_source_bbox_tool_call_reward[data_source].append(bbox_tool_call_reward_tensor[i].item())
                if self.tool_call:
            ...
            for data_source, rewards in data_source_bbox_tool_call_reward.items():
                metric_dict[f'val/test_score/{data_source}/bbox_tool_call_reward'] = np.mean(rewards)
            if self.tool_call:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        reward_tensor, acc_reward_tensor, format_reward_tensor, bbox_tool_call_reward_tensor, invalid_uids = self.reward_fn(new_batch)
                        if self.config.trainer.soft_tool_call_penalty_ratio > 0:
                            ...
                            penalized_accuracy_tensor = reward_tensor - format_reward_tensor - bbox_tool_call_reward_tensor    # Only keep accuracy scores with penalty
                            ...
                            reward_tensor = penalized_accuracy_tensor_.view(-1, max_response_length) + format_reward_tensor + bbox_tool_call_reward_tensor   # Add format scores
                        ...
                        new_batch.batch['bbox_tool_call_scores'] = bbox_tool_call_reward_tensor.sum(-1)
                        ...
                        for uid in unique_uids:
                            ...
                            if (uid_rewards == 0).all():
                                solve_none += 1
                            elif (uid_rewards == self.config.data.acc_reward_weight + self.config.data.format_reward_weight + self.config.data.bbox_tool_call_reward_weight).all():
                                solve_all += 1

                            uid_acc_rewards = acc_reward_tensor[uid_mask].sum(-1)
                            uid_format_rewards = format_reward_tensor[uid_mask].sum(-1)
                            uid_bbox_tool_call_rewards = bbox_tool_call_reward_tensor[uid_mask].sum(-1)
                            ...
                            if (uid_bbox_tool_call_rewards == 0).all():
                                solve_bbox_tool_call_none += 1
                            elif (uid_bbox_tool_call_rewards == self.config.data.bbox_tool_call_reward_weight).all():
                                solve_bbox_tool_call_all += 1
                        ...
                        metrics['batch/solve_bbox_tool_call_none'] = solve_bbox_tool_call_none / totals
                        metrics['batch/solve_bbox_tool_call_all'] = solve_bbox_tool_call_all / totals
