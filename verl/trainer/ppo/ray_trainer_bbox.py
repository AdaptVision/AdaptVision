...
def apply_kl_penalty_to_outcome_reward(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty='kl'):
    responses = data.batch['responses']
    response_length = responses.size(1)
    outcome_level_scores = data.batch['outcome_level_scores']
    batch_size = data.batch.batch_size[0]
    attention_mask = data.batch['attention_mask']
    response_mask = attention_mask[:, -response_length:]
    ...
    outcome_level_rewards = outcome_level_scores - beta * kld
    ...
    # according to https://github.com/huggingface/trl/blob/951ca1841f29114b969b57b26c7d3e80a39f75a0/trl/trainer/ppo_trainer.py#L837
    kl_ctrl.update(current_kl=current_kl, n_steps=batch_size)
    data.batch['outcome_level_rewards'] = outcome_level_rewards
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, lambda_coeff=1.0):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.DTPO: 
        turn_level_rewards = data.batch['turn_level_rewards']
        outcome_level_rewards = data.batch['outcome_level_rewards']
        index = data.non_tensor_batch['uid']
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        turn_mask = turn_mask[:, -response_length:]
        advantages, returns, adv_turn, adv_outcome = core_algos.compute_DTPO_advantage(turn_level_rewards=turn_level_rewards,
                                                                  outcome_level_rewards=outcome_level_rewards,
                                                                  turn_mask=turn_mask,
                                                                  eos_mask=response_mask,
                                                                  index=index,
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
        data.batch['adv_turn'] = adv_turn
        data.batch['adv_outcome'] = adv_outcome
...
def compute_data_metrics(batch, use_critic=True):
    ...
    if 'format_scores' in batch.batch:
        ...
        area_penalty_score = batch.batch['area_penalty_scores']
        bbox_reward = batch.batch['bbox_scores']
    else:
        format_reward = torch.tensor(0.0)
        accuracy_reward = torch.tensor(0.0)
        area_penalty_score = torch.tensor(0.0)
        bbox_reward = torch.tensor(0.0)
    ...
    adv_metrics = {}
    if 'adv_turn' in batch.batch and 'adv_outcome' in batch.batch:
        adv_turn = batch.batch['adv_turn'] # (bs,)
        adv_outcome = batch.batch['adv_outcome'] # (bs,)


        # turn_mask = batch.batch['turn_mask']
        # turn_mask_sliced = turn_mask[:, -response_length:] #(bs, response_length)
        # max_turn_ids, _ = torch.max(turn_mask_sliced, dim=1) #(bs,)
        # is_multi_turn = max_turn_ids > 1 # (bs,)
        # valid_turn_adv = adv_turn[is_multi_turn] # (num_multi_turn, )

        # # Add metrics for turn_adv (only for multi-turn samples)
        # if valid_turn_adv.numel() > 0: # Check if there are any multi-turn samples
        #     adv_metrics.update({
        #         'critic/advantages_turn/mean': torch.mean(valid_turn_adv).detach().item(),
        #         'critic/advantages_turn/max': torch.max(valid_turn_adv).detach().item(),
        #         'critic/advantages_turn/min': torch.min(valid_turn_adv).detach().item(),
        #     })

        # Add metrics for outcome_adv (for all samples)
        adv_metrics.update({
            'critic/advantages_outcome/mean': torch.mean(adv_outcome).detach().item(),
            'critic/advantages_outcome/max': torch.max(adv_outcome).detach().item(),
            'critic/advantages_outcome/min': torch.min(adv_outcome).detach().item(),
        })
        
        adv_metrics.update({
            'critic/advantages_turn/mean': torch.mean(adv_turn).detach().item(),
            'critic/advantages_turn/max': torch.max(adv_turn).detach().item(),
            'critic/advantages_turn/min': torch.min(adv_turn).detach().item(),
        })
    ...
    metrics = {
        ...
        'critic/rewards/mean/area_penalty_score':
            torch.mean(area_penalty_score).detach().item(),
        'critic/rewards/mean/bbox_score':
            torch.mean(bbox_reward).detach().item(),
        'critic/rewards/mean':
...
def compute_data_source_distribution_metrics(batch):
    """计算batch中不同data_source的分布比例"""
    metrics = {}
    ...
    total_samples = len(data_sources)
    ...
    for data_source, count in data_source_counts.items():
        ...
        metrics[f'batch/data_source_ratio/{data_source}'] = ratio
    
    # 添加总的data_source种类数量
    metrics['batch/data_source_types_count'] = len(data_source_counts)
...
class RayPPObboxTrainer(object):
    """
    ...
    def __init__(self,
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True
                ...
                AdvantageEstimator.RLOO, AdvantageEstimator.DTPO
        ]:
    ...
    def _maybe_log_tool_call_metrics(self, batch, metrics):
        ...
        for response, multi_modal_item in zip(responses, multi_modal_data):
            if self.config.actor_rollout_ref.rollout.name in ["vllm_multi_turn_tool_call", "vllm_multi_turn_crop_call", "vllm_multi_turn_multi_tool", "vllm_multi_turn_bbox_call"]:
    ...
    def _validate(self):
        ...
        if self.config.reward_model.log_rewards_separately:
            ...
            area_penalty_score_tensor_lst = []
            area_ratio_tensor_lst = []
            bbox_reward_tensor_lst = []
        ...
        for test_data in self.val_dataloader:
            ...
            if 'multi_modal_data' in test_batch.non_tensor_batch.keys():
                _non_tensor_batch_keys = ['raw_prompt_ids', 'multi_modal_data']
                if self.config.actor_rollout_ref.rollout.debug_image_dir is not None:
                    _non_tensor_batch_keys.append('data_source')
                test_gen_batch = test_batch.pop(
                    batch_keys=['input_ids', 'attention_mask', 'position_ids', 'input_width', 'input_height'],
                    non_tensor_batch_keys=_non_tensor_batch_keys,
                )
            else:
                test_gen_batch = test_batch.pop(
            ...
            reward_tensor, acc_reward_tensor, format_reward_tensor, area_penalty_score_tensor, area_ratio_tensor, bbox_reward_tensor, tool_call_count_tensor, invalid_uids = self.val_reward_fn(test_batch)
            if self.config.reward_model.log_rewards_separately:
                format_reward_tensor_lst.append(format_reward_tensor)
                acc_reward_tensor_lst.append(acc_reward_tensor)
                area_penalty_score_tensor_lst.append(area_penalty_score_tensor)
                area_ratio_tensor_lst.append(area_ratio_tensor)
                bbox_reward_tensor_lst.append(bbox_reward_tensor)
            # Store scores
        ...
        if self.config.reward_model.log_rewards_separately:
            ...
            area_penalty_score_tensor = torch.cat(area_penalty_score_tensor_lst, dim=0).sum(-1).cpu()  # (batch_size,)
            area_ratio_seq = torch.cat(area_ratio_tensor_lst, dim=0).sum(-1).cpu()  # (batch_size,)
            bbox_reward_tensor = torch.cat(bbox_reward_tensor_lst, dim=0).sum(-1).cpu()
            if self.tool_call:
                penalized_acc_reward_tensor = reward_tensor - format_reward_tensor + area_penalty_score_tensor - bbox_reward_tensor


        metric_dict = {}
        # Monitoring metrics for bbox area ratios on validation (aggregated)
        try:
            num_rollouts = self.config.actor_rollout_ref.rollout.n
            area_ratio_tensor_full = torch.cat(area_ratio_tensor_lst, dim=0)
            area_ratio_seq_agg = area_ratio_tensor_full.sum(-1)  # (B,)
            area_ratio_group = area_ratio_seq_agg.view(-1, num_rollouts)
            std_per_question = torch.std(area_ratio_group, dim=1, unbiased=False)
            metric_dict['val/tool_call/bbox_area_ratio_std_per_question_mean'] = torch.mean(std_per_question).item()
            metric_dict['val/tool_call/bbox_area_ratio_batch_mean'] = torch.mean(area_ratio_seq_agg).item()
            # used-only mean on validation: 0 < area_ratio < 1
            used_mask_val = (area_ratio_seq_agg > 0) & (area_ratio_seq_agg < 1)
            if torch.any(used_mask_val):
                metric_dict['val/tool_call/bbox_area_ratio_batch_mean_used_only'] = torch.mean(area_ratio_seq_agg[used_mask_val]).item()
        except Exception:
            pass
        ...
        if self.config.reward_model.log_rewards_separately:
            ...
            data_source_area_penalty_score = {}
            data_source_area_ratio = {}
            data_source_bbox_reward = {}
            if self.tool_call:
            ...
            for i in range(reward_tensor.shape[0]):
                ...
                if data_source not in data_source_area_penalty_score:
                    data_source_area_penalty_score[data_source] = []
                data_source_area_penalty_score[data_source].append(area_penalty_score_tensor[i].item())
                if data_source not in data_source_area_ratio:
                    data_source_area_ratio[data_source] = []
                data_source_area_ratio[data_source].append(area_ratio_seq[i].item())
                if data_source not in data_source_bbox_reward:
                    data_source_bbox_reward[data_source] = []
                data_source_bbox_reward[data_source].append(bbox_reward_tensor[i].item())
                if self.tool_call:
            ...
            for data_source, rewards in data_source_area_penalty_score.items():
                metric_dict[f'val/test_score/{data_source}/area_penalty_score'] = np.mean(rewards)
            for data_source, rewards in data_source_bbox_reward.items():
                metric_dict[f'val/test_score/{data_source}/bbox_reward'] = np.mean(rewards)
            if self.tool_call:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if 'multi_modal_data' in new_batch.non_tensor_batch.keys():
                    _non_tensor_keys = ['raw_prompt_ids', 'multi_modal_data']
                    if self.config.actor_rollout_ref.rollout.debug_image_dir is not None:
                        _non_tensor_keys.append('data_source')
                    gen_batch = new_batch.pop(
                        batch_keys=['input_ids', 'attention_mask', 'position_ids', 'input_width', 'input_height'],
                        non_tensor_batch_keys=_non_tensor_keys,
                    )
                else:
                    gen_batch = new_batch.pop(
                        batch_keys=['input_ids', 'attention_mask', 'position_ids'],
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        with _timer('reward_fn', timing_raw):
                            reward_tensor, acc_reward_tensor, format_reward_tensor, area_penalty_score_tensor, area_ratio_tensor, bbox_reward_tensor, tool_call_count_tensor, invalid_uids = self.reward_fn(new_batch)
                        
                        # 计算ratio (\theta)
                        max_response_length = reward_tensor.shape[-1]
                        penalized_accuracy_tensor = reward_tensor - format_reward_tensor - bbox_reward_tensor + area_penalty_score_tensor    # Only keep accuracy scores with penalty
                        ...
                        correct_direct_answer = (penalized_accuracy_tensor_ == self.config.data.acc_reward_weight).any(dim=2)  # 调用tool有惩罚
                        ...
                        ratio_mask = ratio < self.config.trainer.soft_tool_call_penalty_ratio  # bs  # 问题比较难  ratio < theta, 包含ratio=0的情况 (NOTE)

                        new_acc_reward_tensor = penalized_accuracy_tensor
                        new_area_reward_tensor = area_penalty_score_tensor
                        new_bbox_reward_tensor = bbox_reward_tensor
                        ...
                        if self.config.trainer.soft_tool_call_penalty_ratio > 0:
                            ...
                            # Penalty
                            penalty_tensor_mask = (ratio < self.config.trainer.soft_tool_call_penalty_ratio) & (ratio > 0)  # bs  # 问题比较难  不包含ratio=0的情况 (NOTE)
                            ...
                            correct_direct_answer_item_mask = penalized_accuracy_tensor_ == self.config.data.acc_reward_weight  # (bs, n, max_response_length)

                            # Direct Answer Penalty
                            if self.config.data.direct_answer_penalty > 0:
                                modify_penalty_direct_answer_item_mask = penalty_tensor_mask_expand & correct_direct_answer_item_mask  # (bs, n, max_response_length)
                                # 对于direct正确率低的问题，如果用direct答对了，给惩罚
                                penalized_accuracy_tensor_[modify_penalty_direct_answer_item_mask] = self.config.data.acc_reward_weight - self.config.data.direct_answer_penalty  # (bs, n, max_response_length)
                            reward_tensor = penalized_accuracy_tensor_.view(-1, max_response_length) + format_reward_tensor + bbox_reward_tensor - area_penalty_score_tensor   # Add format scores
                            new_acc_reward_tensor = penalized_accuracy_tensor_.view(-1, max_response_length)

                        if self.config.data.adaptive_area:
                            bbox_reward_tensor_ = bbox_reward_tensor.view(-1, self.config.actor_rollout_ref.rollout.n, max_response_length)  # (bs, n, max_response_length)
                            bbox_effective_item_mask = (bbox_reward_tensor_ > 0).any(dim=2)  # (bs, n)
                            hard_bbox_effective_item_mask = bbox_effective_item_mask & ratio_mask.unsqueeze(1).expand(-1, self.config.actor_rollout_ref.rollout.n)  # (bs, n)  # (r_bbox=1 & ratio < theta)
                            correct_bbox_effective_item_mask = bbox_effective_item_mask & correct_item_mask  # (bs, n)
                            area_ratio_tensor_ = area_ratio_tensor.view(-1, self.config.actor_rollout_ref.rollout.n, max_response_length)  # (bs, n, max_response_length)

                            cond_bbox_effective_item_mask = correct_bbox_effective_item_mask  # (bs, n)  (acc == 1) 只考虑答对的rollout


                            # 计算 hard&bbox_effective样本的正确率 
                            hard_correct_bbox_effective_mask = hard_bbox_effective_item_mask & correct_bbox_effective_item_mask  # (bs, n)  # hard&correct mask
                            correct_in_hard_bbox_effective_items_ratio = hard_correct_bbox_effective_mask.sum() / (hard_bbox_effective_item_mask.float().sum() + 1e-8)  # hard&bbox_effective样本的正确率
                            metrics['tool_call/correct_in_hard_bbox_effective_items_ratio'] = correct_in_hard_bbox_effective_items_ratio  # hard&bbox_effective样本的正确率

                            # 计算 hard&bbox_effective&correct样本的平均面积占比 
                            hard_correct_bbox_effective_area_ratio_tensor = area_ratio_tensor_.sum(-1) * hard_correct_bbox_effective_mask  # (bs, n)
                            arearatio_in_hardcorrect_bbox_effective_items_ratio = hard_correct_bbox_effective_area_ratio_tensor.sum() / (hard_correct_bbox_effective_mask.float().sum() + 1e-8)  # 1  hard&bbox_effective&correct样本的平均面积占比
                            metrics['tool_call/arearatio_in_hardcorrect_bbox_effective_items_ratio'] = arearatio_in_hardcorrect_bbox_effective_items_ratio  # hard&bbox_effective&correct样本的平均面积占比
                            
                            # 对area_ratio_tensor_进行mask
                            masked_area_ratios = area_ratio_tensor_ * cond_bbox_effective_item_mask.unsqueeze(-1).float()  # (bs, n, 1)
                            sum_per_item = masked_area_ratios.sum(dim=[1, 2])  # bs  每个问题，group中hard_bbox_effective的条目的 面积占比总和
                            num_valid_rollouts_per_item = cond_bbox_effective_item_mask.sum(dim=1)  # bs  每个问题，group中hard_bbox_effective的数量
                            indicator_mask = cond_bbox_effective_item_mask.unsqueeze(-1).float()  # (bs, n, 1)
                            
                            mean_conditional_area_ratio_per_item = sum_per_item / (num_valid_rollouts_per_item + 1e-8)  # bs 每个问题，group中correct_bbox_effective的 平均面积占比
                            mean_conditional_area_ratio_per_item = mean_conditional_area_ratio_per_item.unsqueeze(1).unsqueeze(2).expand(-1, self.config.actor_rollout_ref.rollout.n, max_response_length)  # duplicate成 (bs, n, max_response_length)

                            normalized_area_ratio = torch.zeros_like(area_ratio_tensor_)  # (bs, n, max_response_length)
                            area_reward_mask = area_ratio_tensor_ > 0 & cond_bbox_effective_item_mask.unsqueeze(-1).expand(-1, self.config.actor_rollout_ref.rollout.n, max_response_length)  # (bs, n, max_response_length)
                            # 只对有面积，并且是correct_bbox_effective的地方进行normalize
                            normalized_area_ratio[area_reward_mask] = area_ratio_tensor_[area_reward_mask] / (mean_conditional_area_ratio_per_item[area_reward_mask] + 1e-8) - 1.0  # (bs, n, max_response_length)

                            # TODO 验证：mean_conditional_area_ratio_per_item[area_reward_mask] 应该全部都不为0
                            # print ('mean_conditional_area_ratio_per_item[area_reward_mask].min:', mean_conditional_area_ratio_per_item[area_reward_mask].min())
                            # print ('mean_conditional_area_ratio_per_item[area_reward_mask].max:', mean_conditional_area_ratio_per_item[area_reward_mask].max())
                            # print ('mean_conditional_area_ratio_per_item[area_reward_mask].mean:', mean_conditional_area_ratio_per_item[area_reward_mask].mean())
                            # pdb.set_trace()
                            # ray.util.pdb.set_trace()
                            
                            # normalized_area_ratio = area_ratio_tensor_ / mean_conditional_area_ratio_per_item.view(-1, 1, 1) - 1.0  # (bs, n, max_response_length)
                            clipped_value = torch.clamp(normalized_area_ratio, 0, 1)  # (bs, n, max_response_length)  小于均值不惩罚，大于均值惩罚

                            r_area_reward_tensor_ = indicator_mask * clipped_value  # (bs, n, max_response_length)

                            r_area_reward_tensor = r_area_reward_tensor_.view(-1, max_response_length)  # (bs*n, max_response_length)

                            reward_tensor = reward_tensor + area_penalty_score_tensor - r_area_reward_tensor * self.config.data.area_penalty_weight  # (bs, n, max_response_length)
                            new_area_reward_tensor = r_area_reward_tensor * self.config.data.area_penalty_weight


                        # Monitoring metrics for bbox area ratios
                        try:
                            num_rollouts = self.config.actor_rollout_ref.rollout.n
                            area_ratio_seq = area_ratio_tensor.sum(-1)  # (B,)

                            # (a) std across rollouts for each question
                            area_ratio_group = area_ratio_seq.view(-1, num_rollouts)
                            std_per_question = torch.std(area_ratio_group, dim=1, unbiased=False)
                            metrics['tool_call/bbox_area_ratio_std_per_question_mean'] = torch.mean(std_per_question).item()
                            # (b) batch mean of bbox area ratio
                            metrics['tool_call/bbox_area_ratio_batch_mean'] = torch.mean(area_ratio_seq).item()
                            # (c) used-only mean: only samples with 0 < area_ratio < 1 (actual cropped region)
                            used_mask = (area_ratio_seq > 0) & (area_ratio_seq < 1)
                            if torch.any(used_mask):
                                metrics['tool_call/bbox_area_ratio_batch_mean_used_only'] = torch.mean(area_ratio_seq[used_mask]).item()
                        except Exception:
                            pass
                        ...
                        new_batch.batch['area_penalty_scores'] = area_penalty_score_tensor.sum(-1)
                        new_batch.batch['area_ratio'] = area_ratio_tensor.sum(-1)
                        new_batch.batch['bbox_scores'] = bbox_reward_tensor.sum(-1)
                        
                        new_batch.batch['outcome_level_scores'] = new_acc_reward_tensor + format_reward_tensor

                        new_batch.batch['turn_level_scores'] = new_bbox_reward_tensor - new_area_reward_tensor
                        uids = new_batch.non_tensor_batch['uid']
                        unique_uids = np.unique(uids)
                        valid_mask = torch.ones(len(uids), dtype=torch.bool)

                        metrics['train/new_bbox_reward/mean'] = new_bbox_reward_tensor.sum(-1).mean()
                        metrics['train/new_bbox_reward/min'] = new_bbox_reward_tensor.sum(-1).min()
                        metrics['train/new_bbox_reward/max'] = new_bbox_reward_tensor.sum(-1).max()
                        metrics['train/new_area_reward/mean'] = new_area_reward_tensor.sum(-1).mean()
                        metrics['train/new_area_reward/min'] = new_area_reward_tensor.sum(-1).min()
                        metrics['train/new_area_reward/max'] = new_area_reward_tensor.sum(-1).max()
                        ...
                        for uid in unique_uids:
                            ...
                            if (uid_rewards == 0).all():
                                solve_none += 1
                            elif (uid_rewards == self.config.data.acc_reward_weight + self.config.data.format_reward_weight - self.config.data.area_penalty_weight + self.config.data.bbox_reward_weight).all():
                                solve_all += 1

                            uid_acc_rewards = acc_reward_tensor[uid_mask].sum(-1)
                            uid_format_rewards = format_reward_tensor[uid_mask].sum(-1)
                            uid_bbox_rewards = bbox_reward_tensor[uid_mask].sum(-1)
                            # uid_area_penalty_scores = area_penalty_score_tensor[uid_mask].sum(-1)
                            ...
                            if (uid_bbox_rewards == 0).all():
                                solve_bbox_none += 1
                            elif (uid_bbox_rewards == self.config.data.bbox_reward_weight).all():
                                solve_bbox_all += 1
                        ...
                        metrics['batch/solve_bbox_none'] = solve_bbox_none / totals
                        metrics['batch/solve_bbox_all'] = solve_bbox_all / totals
                        ...
                        #     if self.config.algorithm.adv_estimator == AdvantageEstimator.MTGRPO:
                        #         batch, kl_metrics = apply_kl_penalty_to_outcome_reward(batch,
                        ...
                        batch.batch['outcome_level_rewards'] = batch.batch['outcome_level_scores']
                        batch.batch['turn_level_rewards'] = batch.batch['turn_level_scores']
                    
                                                
                        # 添加不同data_source中的area_ratio和工具调用比例统计
                        if 'data_source' in batch.non_tensor_batch and 'area_ratio' in batch.batch:
                            data_sources = batch.non_tensor_batch['data_source']
                            area_ratios = batch.batch['area_ratio']
                            ...
                            for i, data_source in enumerate(data_sources):
                                ...
                                # 收集area_ratio
                                area_ratio = area_ratios[i].item() if hasattr(area_ratios[i], 'item') else area_ratios[i]
                                data_source_stats[data_source]['area_ratios'].append(area_ratio)
                        
                                # 收集工具调用信息（通过multi_modal_data中的图像数量判断）
                                if multi_modal_data is not None and i < len(multi_modal_data):
                                    images = multi_modal_data[i].get('image', [])
                                    tool_call_success = len(images) > 1
                                    data_source_stats[data_source]['tool_calls'].append(tool_call_success)
                    
                            # 计算每个data_source的统计指标
                            for data_source, stats in data_source_stats.items():
                                area_ratios_list = stats['area_ratios']
                                tool_calls_list = stats['tool_calls']
                        
                                if area_ratios_list:
                                    # Area ratio统计
                                    area_ratios_tensor = torch.tensor(area_ratios_list, dtype=torch.float32)
                                    metrics[f'batch/data_source/{data_source}/area_ratio_mean'] = float(torch.mean(area_ratios_tensor))
                            
                                    # 有效area_ratio统计（0 < area_ratio <=1）
                                    valid_mask = (area_ratios_tensor > 0) & (area_ratios_tensor <= 1)
                                    valid_area_ratios = area_ratios_tensor[valid_mask]
                                    if len(valid_area_ratios) > 0:
                                        metrics[f'batch/data_source/{data_source}/area_ratio_mean_valid'] = float(torch.mean(valid_area_ratios))
                        
                                if tool_calls_list:
                                 # 工具调用比例统计
                                    tool_calls_tensor = torch.tensor(tool_calls_list, dtype=torch.float32)
                                    tool_call_ratio = float(torch.mean(tool_calls_tensor))
                                    metrics[f'batch/data_source/{data_source}/tool_call_ratio'] = tool_call_ratio
                        ...
                        batch = compute_advantage(batch,
                                                  ...
                                                  lambda_coeff=self.config.data.lambda_coeff)
                ...
                metrics.update(compute_data_source_distribution_metrics(batch=batch))
                ...
                success_tool_call_per_batch = 0
                success_tool_call_per_sample = 0
                # solve_bbox_tool_call_none = 0
                # solve_bbox_tool_call_all = 0
                totals = 0
                invalid_num = 0
                effective_num = 0
