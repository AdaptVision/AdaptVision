...
from verl.trainer.ppo.ray_trainer_bbox import RayPPObboxTrainer
...
def main_task(config, compute_score=None):
    ...
    elif reward_manager_name == 'naive_multithreads_bbox':
        from verl.workers.reward_manager import NaiveMultiThreadsBboxRewardManager
        reward_manager_cls = NaiveMultiThreadsBboxRewardManager
    ...
    extra_info = {
        ...
        "bbox_tool_call_reward_weight": config.data.get("bbox_tool_call_reward_weight", 0.0),
        "use_tool_reward_weight": config.data.get("use_tool_reward_weight", 0.0),
        "tool_call_penalty": config.data.get("tool_call_penalty", 0.0),
        ...
        "area_penalty_weight": config.data.get("area_penalty_weight", 0.0),
        "acc_condition": config.data.get("acc_condition", False),
        "min_area_ratio": config.data.get("min_area_ratio", 0.0),
        "bbox_reward_weight": config.data.get("bbox_reward_weight", 0.0),
    }
    ...
    elif config.data.get("val_general_qa_reward_fn", None) == "general_qa_bbox":
        from verl.workers.reward_manager import NaiveMultiThreadsBboxRewardManager
        val_reward_manager_cls = NaiveMultiThreadsBboxRewardManager
        val_reward_fn = val_reward_manager_cls(tokenizer=tokenizer, num_examine=1, compute_score=compute_score, mode="val", extra_info=extra_info)
    else:
    ...
    if config.data.general_qa_reward_fn == "general_qa_bbox":
        trainer_cls = RayPPObboxTrainer
    else:
